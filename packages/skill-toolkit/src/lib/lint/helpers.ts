...
/**
 * Build a failing CheckResult. Accepts `fix: string | undefined` (not
 * just optional) so callers can forward `fix` through without
 * conditional spreads under `exactOptionalPropertyTypes: true`.
 */
...
export function checkFieldNonEmpty({
  ...
  return ({ frontmatter }: SkillRecord): CheckResult =>
    match(isEmpty(frontmatter[field]))
      .with(true, () => fail({ message: `Frontmatter is missing \`${field}\``, fix }))
      .otherwise(() => pass)
}
...
export function checkFieldPresent({
  ...
  return ({ frontmatter }: SkillRecord): CheckResult =>
    match(frontmatter[field])
      .with(undefined, () => fail({ message, fix }))
      .otherwise(() => pass)
}
...
export function checkDescriptionMatches({
  ...
  return ({ frontmatter }: SkillRecord): CheckResult =>
    match(pattern.test(frontmatter.description))
      .with(true, () => pass)
 ...
 * patterns whose presence is forbidden. The matched fragment is passed
 * to `message` so the finding can quote it back ("contains \"first\"").
 */

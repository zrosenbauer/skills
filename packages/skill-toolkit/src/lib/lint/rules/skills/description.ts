...
/**
 * `fm-missing-description` owns the "description is missing or not a
 * string" case. Every rule in this ruleset short-circuits on
 * non-string input so the user only ever sees one finding for the
 * underlying problem.
 */
...
export default defineRuleset({
  ...
  rules: [
    defineRule({
      ...
      check: ({ frontmatter }) => {
        const desc = asString(frontmatter.description)
        if (desc === null) return pass()
        return match(desc.length)
          .when(
            (len) => len >= 80,
            () => pass()
          )
    ...
    defineRule({
      ...
      check: ({ frontmatter }) => {
        const desc = asString(frontmatter.description)
        if (desc === null) return pass()
        return match((desc.match(QUOTED_PHRASE_RE) ?? []).length)
          .when(
            (count) => count >= 3,
            () => pass()
          )
    ...
    defineRule({
      ...
      check: ({ frontmatter }) => {
        const desc = asString(frontmatter.description)
        if (desc === null) return pass()
        return match(desc.match(ANTI_SHORTCUT_RE))
          .with(P.nullish, () => pass())
          .otherwise((m) =>
            fail({

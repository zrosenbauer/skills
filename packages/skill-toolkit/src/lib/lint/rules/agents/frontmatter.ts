 ...
 *
 * Rule ids are unprefixed (`file-name`, `fm-invalid-yaml`, ...). The
...
export default defineRuleset<AgentRecord>({
  ...
  rules: [
    ...
    defineRule<AgentRecord>({
      ...
      check: (agent) =>
        match(agent.frontmatterYamlError)
          .with(P.nullish, () => pass())
          .otherwise((err) =>
            fail({
    ...
    defineRule<AgentRecord>({
      id: 'fm-missing-name',
      severity: 'error',
      description: 'agent frontmatter must include `name` as a non-empty string',
      check: (agent) =>
    ...
    defineRule<AgentRecord>({
      ...
      check: (agent) => {
        ...
        if (name === agent.location.name) return pass()
        return fail({
    ...
    defineRule<AgentRecord>({
      ...
      severity: 'error',
      description: 'agent frontmatter must include `description` as a non-empty string',
      check: (agent) =>
